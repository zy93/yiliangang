...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, -64, self.view.ml_width + ZLPickerColletionViewPadding,self.view.ml_height) collectionViewLayout:flowLayout];
...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        [deleleBtn setImage:[UIImage imageNamed:MLSelectPhotoSrcName(@"icon_image_yes")] forState:UIControlStateNormal];
...
- (void)refreshAsset{
    ...
    if([[self.deleteAssets allValues] count] == 0 || [self.deleteAssets valueForKeyPath:[NSString stringWithFormat:@"%ld",(self.currentPage)]] == nil){
        [self.deleleBtn setImage:[UIImage imageNamed:MLSelectPhotoSrcName(@"icon_image_no")] forState:UIControlStateNormal];
    }else{
...
- (void) pickerPhotoScrollViewDidSingleClick:(MLSelectPhotoPickerBrowserPhotoScrollView *)photoScrollView{
    ...
    self.collectionView.frame = CGRectMake(0, self.navigationController.navigationBarHidden == YES ? 0 : - 64, self.collectionView.ml_width, self.collectionView.ml_height);
    if (self.isEditing) {
