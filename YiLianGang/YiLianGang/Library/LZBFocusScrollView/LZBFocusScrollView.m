    ...
    if(self.images.count > 1)
    {
        switch (self.style) {
            ...
            case LZBFocusScrollViewScrollStyle_oglFlip:
            {
                ...
                [self addGestureRecognizer:fromRightRecognizer];
                ...
                [self addGestureRecognizer:fromLeftRecognizer];
            }
                break;
...
{
    if (self.isLeft) {
        self.nextIndex = (self.currentIndex + self.imageArray.count - 1)%self.imageArray.count;
    }else{
...
- (void)nextImage
{
    ...
    [self performSelector:@selector(animationOver) withObject:nil afterDelay:0.5];
    switch (self.style) {
        case LZBFocusScrollViewScrollStyle_Fade:
            {
